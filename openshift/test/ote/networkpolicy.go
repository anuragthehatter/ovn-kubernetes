...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 49076-service domain can be resolved when egress type is enabled", func() {
		var (
		...
		oteutils.CreateResourceFromFile(oc, oc.Namespace(), testPodFile)
		oteutils.CreateResourceFromFile(oc, oc.Namespace(), helloSdnFile)
		err := oteutils.WaitForPodWithLabelReady(oc, oc.Namespace(), "name=test-pods")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=test-pods not ready")
		err = oteutils.WaitForPodWithLabelReady(oc, oc.Namespace(), "name=hellosdn")
		...
		oteutils.CreateResourceFromFile(oc, oc.Namespace(), egressTypeFile)
		output, err := oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("allow-all-egress"))
		oteutils.CreateResourceFromFile(oc, oc.Namespace(), ingressTypeFile)
		output, err = oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		helloSdnName := oteutils.GetPodName(oc, oc.Namespace(), "name=hellosdn")
		...
		testPodName := oteutils.GetPodName(oc, oc.Namespace(), "name=test-pods")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 49186-Networkpolicy egress rule should work for statefulset pods.", func() {
		var (
			buildPruningBaseDir  = testdata.FixturePath("networking")
			testPodFile          = filepath.Join(buildPruningBaseDir, "testpod.yaml")
		...
		oteutils.CreateResourceFromFile(oc, ns1, helloStatefulsetFile)
		...
		helloPodName := oteutils.GetPodName(oc, ns1, "app=hello")

		g.By("3. Create networkpolicy with egress rule in first namespace.")
		oteutils.CreateResourceFromFile(oc, ns1, egressTypeFile)
		output, err := oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		oteutils.CreateResourceFromFile(oc, ns2, testPodFile)
		err = oteutils.WaitForPodWithLabelReady(oc, oc.Namespace(), "name=test-pods")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=test-pods not ready")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		testPodName := oteutils.GetPodName(oc, ns2, "name=test-pods")
		err = oteutils.LabelPod(oc, ns2, testPodName[0], "type=red")
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("6. Get IP of the test pods in second namespace.")
		testPodIP1 := oteutils.GetPodIPv4(oc, ns2, testPodName[0])
		testPodIP2 := oteutils.GetPodIPv4(oc, ns2, testPodName[1])
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 49437-Ingress network policy shouldn't be overruled by egress network policy on another pod", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
		...
		podns1 := oteutils.PingPodResourceNode{
			Name:      "hello-pod",
			Namespace: ns1,
		...
		oteutils.WaitPodReady(oc, podns1.Namespace, podns1.Name)
		...
		pod1Ns2 := oteutils.PingPodResourceNode{
			Name:      "hello-pod",
			Namespace: ns2,
			Nodename:  nodeList.Items[0].Name,
			Template:  pingPodNodeTemplate,
		...
		oteutils.WaitPodReady(oc, pod1Ns2.Namespace, pod1Ns2.Name)

		g.By("create another hello-pod on 2nd namesapce but on different node")
		pod2Ns2 := oteutils.PingPodResourceNode{
		...
		oteutils.WaitPodReady(oc, pod2Ns2.Namespace, pod2Ns2.Name)

		helloPodNameNs2 := oteutils.GetPodName(oc, ns2, "name=hello-pod")

		g.By("create default deny ingress type networkpolicy in 2nd namespace")
		oteutils.CreateResourceFromFile(oc, ns2, ingressTypeFile)
		output, err = oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("default-deny-ingress"))

		g.By("3. Get IP of the test pods in second namespace.")
		hellopodIP1Ns2 := oteutils.GetPodIPv4(oc, ns2, helloPodNameNs2[0])
		hellopodIP2Ns2 := oteutils.GetPodIPv4(oc, ns2, helloPodNameNs2[1])
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 49686-network policy with ingress rule with ipBlock", func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
		...
		pod3ns1 := oteutils.PingPodResourceNode{
			Name:      "hello-pod3",
			Namespace: ns1,
			Nodename:  nodeList.Items[1].Name,
			Template:  pingPodNodeTemplate,
		...
		oteutils.WaitPodReady(oc, pod3ns1.Namespace, pod3ns1.Name)

		helloPod1ns1IPv6, helloPod1ns1IPv4 := oteutils.GetPodIP(oc, ns1, pod1ns1.Name)
		...
		if ipStackType == "dualstack" {
			...
			npIPBlockNS1 := oteutils.IpBlockCIDRsDual{
			...
			npIPBlockNS1 := oteutils.IpBlockCIDRsSingle{
		...
		oteutils.CurlPod2PodPass(oc, ns1, "hello-pod1", ns1, "hello-pod3")

		g.By("Checking connectivity from pod2 to pod3")
		oteutils.CurlPod2PodFail(oc, ns1, "hello-pod2", ns1, "hello-pod3")
		...
		oteutils.CurlPod2PodFail(oc, ns2, "hello-pod1", ns1, "hello-pod3")

		g.By("Checking connectivity from pod2ns2 to pod1ns1")
		oteutils.CurlPod2PodFail(oc, ns2, "hello-pod2", ns1, "hello-pod1")
		...
		helloPod2ns2IPv6, helloPod2ns2IPv4 := oteutils.GetPodIP(oc, ns2, pod2ns2.Name)
		...
		if ipStackType == "dualstack" {
			...
			npIPBlockNS1New := oteutils.IpBlockCIDRsDual{
				Name:      "ipblock-dual-cidrs-ingress",
				Template:  ipBlockIngressTemplateDual,
			...
			npIPBlockNS1New := oteutils.IpBlockCIDRsSingle{
				Name:      "ipblock-single-cidr-ingress",
				Template:  ipBlockIngressTemplateSingle,
		...
		oteutils.CurlPod2PodPass(oc, ns2, "hello-pod2", ns1, "hello-pod3")
		...
		oteutils.CurlPod2PodPass(oc, ns1, "hello-pod2", ns1, "hello-pod3")

		g.By("Checking connectivity from pod1ns2 to pod3ns1")
		oteutils.CurlPod2PodPass(oc, ns2, "hello-pod1", ns1, "hello-pod3")

		g.By("Checking connectivity from pod2ns2 to pod1ns1 on IPv4 interface")
		oteutils.CurlPod2PodPass(oc, ns2, "hello-pod2", ns1, "hello-pod1")

	})

	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 49696-mixed ingress and egress policies can work well", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
			testPodFile         = filepath.Join(buildPruningBaseDir, "testpod.yaml")
			helloSdnFile        = filepath.Join(buildPruningBaseDir, "hellosdn.yaml")
		...
		oteutils.CreateResourceFromFile(oc, ns1, helloSdnFile)
		err := oteutils.WaitForPodWithLabelReady(oc, ns1, "name=test-pods")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=test-pods not ready")
		err = oteutils.WaitForPodWithLabelReady(oc, ns1, "name=hellosdn")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=hellosdn not ready")
		hellosdnPodNameNs1 := oteutils.GetPodName(oc, ns1, "name=hellosdn")
		...
		oteutils.CreateResourceFromFile(oc, ns2, helloSdnFile)
		err = oteutils.WaitForPodWithLabelReady(oc, ns2, "name=hellosdn")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=hellosdn not ready")

		g.By("Get IP of the test pods in second namespace.")
		hellosdnPodNameNs2 := oteutils.GetPodName(oc, ns2, "name=hellosdn")
		hellosdnPodIP1Ns2 := oteutils.GetPodIPv4(oc, ns2, hellosdnPodNameNs2[0])
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 46246-Network Policies should work with OVNKubernetes when traffic hairpins back to the same source through a service", func() {
		var (
			buildPruningBaseDir    = testdata.FixturePath("networking")
			pingPodNodeTemplate    = filepath.Join(buildPruningBaseDir, "ping-for-pod-specific-node-template.yaml")
		...
		oteutils.WaitPodReady(oc, ns, pod1.Name)
		...
		oteutils.WaitPodReady(oc, ns, pod2.Name)

		g.By("Create a test service backing up both the above pods")
		svc := oteutils.GenericServiceResource{
		...
		oteutils.CreateResourceFromFile(oc, ns, allowfromsameNS)

		g.By("curl from hello-pod1 to hello-pod2")
		oteutils.CurlPod2PodPass(oc, ns, "hello-pod1", ns, "hello-pod2")

		g.By("curl from hello-pod2 to hello-pod1")
		oteutils.CurlPod2PodPass(oc, ns, "hello-pod2", ns, "hello-pod1")

		for i := 0; i < 5; i++ {

			g.By("curl from hello-pod1 to service:port")
			oteutils.CurlPod2SvcPass(oc, ns, ns, "hello-pod1", "test-service")

			g.By("curl from hello-pod2 to service:port")
			oteutils.CurlPod2SvcPass(oc, ns, ns, "hello-pod2", "test-service")
		}

		g.By("Make sure pods are curl'able from respective nodes")
		oteutils.CurlNode2PodPass(oc, pod1.Nodename, ns, "hello-pod1")
		oteutils.CurlNode2PodPass(oc, pod2.Nodename, ns, "hello-pod2")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 41879-ipBlock should not ignore all other cidr's apart from the last one specified", func() {
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
		...
		testPod1IPv6, testPod1IPv4 := oteutils.GetPodIP(oc, ns1, testPodName[0])
		testPod1IPv4WithCidr := testPod1IPv4 + "/32"
		testPod1IPv6WithCidr := testPod1IPv6 + "/128"
		testPod2IPv6, testPod2IPv4 := oteutils.GetPodIP(oc, ns1, testPodName[1])
		testPod2IPv4WithCidr := testPod2IPv4 + "/32"
		testPod2IPv6WithCidr := testPod2IPv6 + "/128"
		testPod3IPv6, testPod3IPv4 := oteutils.GetPodIP(oc, ns1, testPodName[2])
		...
		oteutils.CurlPod2PodPass(oc, ns1, testPodName[0], ns1, testPodName[4])

		g.By("Checking connectivity from pod2 to pod5")
		oteutils.CurlPod2PodPass(oc, ns1, testPodName[1], ns1, testPodName[4])

		g.By("Checking connectivity from pod3 to pod5")
		oteutils.CurlPod2PodPass(oc, ns1, testPodName[2], ns1, testPodName[4])

		g.By("Checking connectivity from pod4 to pod5")
		oteutils.CurlPod2PodFail(oc, ns1, testPodName[3], ns1, testPodName[4])

	})

	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 46807-network policy with egress rule with ipBlock", func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
		...
		helloPod1ns1IP1, helloPod1ns1IP2 := oteutils.GetPodIP(oc, ns1, pod1ns1.Name)
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 46808-network policy with egress rule with ipBlock and except", func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
		...
		pod4ns1 := oteutils.PingPodResourceNode{
			Name:      "hello-pod4",
			Namespace: ns1,
			Nodename:  nodeList.Items[1].Name,
			Template:  pingPodNodeTemplate,
		...
		oteutils.WaitPodReady(oc, pod4ns1.Namespace, pod4ns1.Name)

		helloPod2ns1IP1, helloPod2ns1IP2 := oteutils.GetPodIP(oc, ns1, pod2ns1.Name)
		if ipStackType == "dualstack" {
			hostSubnetCIDRIPv4, hostSubnetCIDRIPv6 := oteutils.GetNodeSubnetDualStack(oc, nodeList.Items[0].Name, "default")
			...
			npIPBlockNS1 := oteutils.IpBlockCIDRsExceptDual{
			...
			if ipStackType == "ipv6single" {
				hostSubnetCIDRIPv6 := oteutils.GetNodeSubnet(oc, nodeList.Items[0].Name, "default")
				o.Expect(hostSubnetCIDRIPv6).NotTo(o.BeEmpty())
				helloPod2ns1IPv6WithCidr := helloPod2ns1IP1 + "/128"
				g.By("create ipBlock Egress CIDRs with except rule Policy in ns1 on IPv6 singlestack")
				npIPBlockNS1 := oteutils.IpBlockCIDRsExceptSingle{
				...
				hostSubnetCIDRIPv4 := oteutils.GetNodeSubnet(oc, nodeList.Items[0].Name, "default")
				o.Expect(hostSubnetCIDRIPv4).NotTo(o.BeEmpty())
		...
		o.Eventually(func() error {
			podIP1, podIP2 := oteutils.GetPodIP(oc, ns1, "hello-pod1")
		...
		oteutils.CurlPod2PodFail(oc, ns1, "hello-pod3", ns1, "hello-pod2")

		g.By("Checking connectivity from pod3 to pod4")
		oteutils.CurlPod2PodFail(oc, ns1, "hello-pod3", ns1, "hello-pod4")
		if ipStackType == "dualstack" {
			g.By("Delete networkpolicy from ns1 so no networkpolicy in namespace")
		...
		g.By("Checking connectivity from pod3 to pod1")
		oteutils.CurlPod2PodPass(oc, ns1, "hello-pod3", ns1, "hello-pod1")

		g.By("Checking connectivity from pod3 to pod2")
		oteutils.CurlPod2PodPass(oc, ns1, "hello-pod3", ns1, "hello-pod2")

		g.By("Checking connectivity from pod3 to pod4")
		oteutils.CurlPod2PodPass(oc, ns1, "hello-pod3", ns1, "hello-pod4")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][WRS][V-BR.33] 41080-Check network policy ACL audit messages are logged to journald", g.Serial, func() {
		...
		cmd := "journalctl -t ovn-controller --since '1min ago'| grep 'verdict=allow'"
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 55287-Default network policy ACLs to a namespace should not be present with arp but arp||nd for ARPAllowPolicies", func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
		...
		listOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnMasterPodName, "ovnkube-controller", listACLCmd)
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 62524-OVN address_set referenced in acl should not miss when networkpolicy name includes dot.", func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
		...
		defer func() {
			if os.Getenv("DELETE_NAMESPACE") != "false" {
				_ = oc.AsAdmin().WithoutNamespace().Run("label").Args("ns", ns, "team-").Execute()
			}
		}()
		...
		g.By("create test pods")
		oteutils.CreateResourceFromFile(oc, ns, testPodFile)
		err = oteutils.WaitForPodWithLabelReady(oc, ns, "name=test-pods")
		o.Expect(err).NotTo(o.HaveOccurred(), "this pod with label name=test-pods not ready")
		testPod := oteutils.GetPodName(oc, ns, "name=test-pods")
		...
		oteutils.CreateResourceFromFile(oc, ns, networkPolicyFile)
		output, err := oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		for _, addrSetName := range result {
			...
			listAddrOutput, listAddrErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnMasterPodName, "ovnkube-controller", listAddressSetCmd)
			o.Expect(listAddrErr).NotTo(o.HaveOccurred())
			o.Expect(listAddrOutput).NotTo(o.BeEmpty())
		}

		g.By("Checking pods connectivity")
		oteutils.CurlPod2PodPass(oc, ns, testPod[0], ns, pod1.Name)
		oteutils.CurlPod2PodFail(oc, ns, testPod[0], ns, testPod[1])

	})

	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 65901-Duplicate transactions should not be executed for network policy for every pod update.", g.Serial, func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
		...
		podIP1, _ := oteutils.GetPodIP(oc, ns, pod.Name)

		podNodeName, podNodenameErr := oteutils.GetPodNodeName(oc, ns, pod.Name)
		...
		ovnKNodePod, ovnkNodePodErr := oteutils.GetOVNKPodOnNode(oc, "openshift-ovn-kubernetes", "app=ovnkube-node", podNodeName)
		...
		g.By("Get the PodSelector address set UUID for the 'type=red' peer selector")
		getAddrSetUUIDCmd := fmt.Sprintf("ovn-nbctl --columns=_uuid --bare --no-leader-only find address_set 'external_ids:k8s.ovn.org/owner-type=PodSelector' 'external_ids:k8s.ovn.org/name=\"LS{ML:{kubernetes.io/metadata.name: %s,},}_LS{ML:{type: red,},}\"' 'addresses{>=}\"%s\"'", ns, podIP1)
		var addressSetUUID string
		o.Eventually(func() string {
			result, err := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", getAddrSetUUIDCmd)
			if err != nil {
				e2e.Logf("Error querying OVN NB for PodSelector address set: %v", err)
				return ""
			}
			result = strings.TrimSpace(result)
			// Dual-stack clusters may return both v4 and v6 address sets; use the first one
			if strings.Contains(result, "\n") {
				result = strings.Split(result, "\n")[0]
			}
			addressSetUUID = result
			return addressSetUUID
		}, 30*time.Second, 2*time.Second).ShouldNot(o.BeEmpty(), "PodSelector address set UUID for selector 'type=red' in namespace %s should not be empty", ns)
		e2e.Logf("PodSelector address set UUID for selector 'type=red': %s", addressSetUUID)

		g.By("Verify the address set belongs to the test network policy's peer podSelector")
		getAddrSetExternalIDsCmd := fmt.Sprintf("ovn-nbctl get address_set %s external_ids", addressSetUUID)
		addrSetExternalIDs, externalIDsErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", getAddrSetExternalIDsCmd)
		o.Expect(externalIDsErr).NotTo(o.HaveOccurred())
		addrSetExternalIDs = strings.TrimSpace(addrSetExternalIDs)
		e2e.Logf("Address set external_ids for pod IP %s: %s", podIP1, addrSetExternalIDs)
		o.Expect(addrSetExternalIDs).To(o.ContainSubstring("PodSelector"), "Address set should have owner-type PodSelector")
		o.Expect(addrSetExternalIDs).To(o.ContainSubstring("type: red"), "Address set name should contain the peer podSelector label 'type: red'")

		getLogSizeCmd := "wc -l < /var/log/ovnkube/libovsdb.log"
		logSizeStr, logSizeErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", getLogSizeCmd)
		...
		newTxnCountStr, newTxnErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", getNewTxnCmd)
		...
		if newTxnCountStr != "0" {
			...
			newTxnContent, _ := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", getNewTxnContentCmd)
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 66085-Creating egress network policies for allowing to same namespace and openshift dns in namespace prevents the pod from reaching its own service", func() {
		var (
			buildPruningBaseDir        = testdata.FixturePath("networking")
			pingPodTemplate            = filepath.Join(buildPruningBaseDir, "ping-for-pod-template.yaml")
			genericServiceTemplate     = filepath.Join(buildPruningBaseDir, "service-generic-template.yaml")
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64787-Network policy with duplicate egress rules (same CIDR block) fails to be recreated", g.Serial, func() {
		var (
			buildPruningBaseDir         = testdata.FixturePath("networking")
		...
		podns := oteutils.PingPodResourceNode{
			Name:      "hello-pod",
			Namespace: ns,
			Nodename:  nodeList.Items[0].Name,
			Template:  pingPodNodeTemplate,
		...
		oteutils.WaitPodReady(oc, podns.Namespace, podns.Name)

		helloPodnsIP1, helloPodnsIP2 := oteutils.GetPodIP(oc, ns, podns.Name)
		...
		if ipStackType == "dualstack" {
			...
			npIPBlockNS := oteutils.IpBlockCIDRsDual{
			...
			var npIPBlockNS oteutils.IpBlockCIDRsSingle
			if ipStackType == "ipv6single" {
				helloPodnsIPv6WithCidr := helloPodnsIP1 + "/128"
				npIPBlockNS = oteutils.IpBlockCIDRsSingle{
		...
		ovnKNodePod, ovnkNodePodErr := oteutils.GetOVNKPodOnNode(oc, "openshift-ovn-kubernetes", "app=ovnkube-node", nodeList.Items[0].Name)
		o.Expect(ovnkNodePodErr).NotTo(o.HaveOccurred())
		...
		defer func() { _ = oteutils.WaitForPodWithLabelReady(oc, "openshift-ovn-kubernetes", "app=ovnkube-node") }()
		...
		err = oteutils.WaitForPodWithLabelReady(oc, "openshift-ovn-kubernetes", "app=ovnkube-node")
		o.Expect(err).NotTo(o.HaveOccurred())
		ovnKNodePod, ovnkNodePodErr = oteutils.GetOVNKPodOnNode(oc, "openshift-ovn-kubernetes", "app=ovnkube-node", nodeList.Items[0].Name)
		o.Expect(ovnkNodePodErr).NotTo(o.HaveOccurred())
		o.Expect(ovnKNodePod).ShouldNot(o.Equal(""))
		...
		logContents, logErr := oteutils.GetSpecificPodLogs(oc, "openshift-ovn-kubernetes", "ovnkube-controller", ovnKNodePod, filterString)
		o.Expect(logErr).NotTo(o.HaveOccurred())
		e2e.Logf("Log contents \n%s", logContents)
		o.Expect(strings.Contains(logContents, "failed")).To(o.BeFalse())

	})
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64786-Network policy in namespace that has long name fails to be recreated as the ACLs are considered duplicate", g.Serial, func() {
		var (
		...
		nodeList, err := e2enode.GetReadySchedulableNodes(context.TODO(), oc.KubeFramework().ClientSet)
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(nodeList.Items).NotTo(o.BeEmpty())
		...
		defer func() {
			if os.Getenv("DELETE_NAMESPACE") != "false" {
				_ = oc.AsAdmin().Run("delete").Args("project", testNs, "--ignore-not-found").Execute()
			}
		}()
		defer func() {
		...
		g.By("Create a hello pod in namspace")
		podns := oteutils.PingPodResource{
			Name:      "hello-pod",
			Namespace: testNs,
			Template:  pingPodTemplate,
		}
		...
		oteutils.CreateResourceFromFile(oc, testNs, allowToNSNetworkPolicyFile)
		checkErr := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 100*time.Second, true, func(_ context.Context) (bool, error) {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 64788-Same network policies across multiple namespaces fail to be recreated.", g.Serial, func() {
		if oteutils.IsHypershiftHostedCluster(oc) {
			g.Skip("Not applicable to HyperShift hosted cluster")
		}
		var (
		...
		testPod := oteutils.GetPodName(oc, ns, "name=test-pods")
		nodeName, err := oteutils.GetPodNodeName(oc, ns, testPod[0])
		o.Expect(err).NotTo(o.HaveOccurred())

		helloPod1ns1IPv6, helloPod1ns1IPv4 := oteutils.GetPodIP(oc, ns, testPod[0])
		helloPod1ns1IPv4WithCidr := helloPod1ns1IPv4 + "/32"
		helloPod1ns1IPv6WithCidr := helloPod1ns1IPv6 + "/128"
		...
		ovnKNodePod, ovnkNodePodErr := oteutils.GetOVNKPodOnNode(oc, "openshift-ovn-kubernetes", "app=ovnkube-node", nodeName)
		o.Expect(ovnkNodePodErr).NotTo(o.HaveOccurred())
		o.Expect(ovnKNodePod).ShouldNot(o.Equal(""))
		...
		listAclOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", listACLCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		o.Expect(listAclOutput).NotTo(o.BeEmpty())
		e2e.Logf("%s", listAclOutput)
		...
		if ipStackType == "dualstack" {
			...
			aclMap = oteutils.NbContructToMap(listAcls[0])
			o.Expect(aclMap).NotTo(o.BeEmpty())
			aclMap1 := oteutils.NbContructToMap(listAcls[1])
			o.Expect(aclMap1).NotTo(o.BeEmpty())
			listPGCmd = fmt.Sprintf("ovn-nbctl find port-group acls='[%s, %s]'", aclMap["_uuid"], aclMap1["_uuid"])
		} else {
			aclMap = oteutils.NbContructToMap(listAclOutput)
			o.Expect(aclMap).NotTo(o.BeEmpty())
		...
		listPGOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", listPGCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		o.Expect(listPGOutput).NotTo(o.BeEmpty())
		e2e.Logf("%s", listPGOutput)
		pgMap := oteutils.NbContructToMap(listPGOutput)
		o.Expect(pgMap).NotTo(o.BeEmpty())

		defer func() {
			if os.Getenv("DELETE_NAMESPACE") != "false" {
				_ = oc.AsAdmin().WithoutNamespace().Run("delete").Args("networkpolicy", policyName, "-n", ns).Execute()
			}
		}()
		...
		idOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", createAclCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		...
		_, listErr = oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", setAclPropertiesCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		...
		_, listErr = oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", setAclNameCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		...
		listDupAclOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnKNodePod, "ovnkube-controller", listDupACLCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		o.Expect(listDupAclOutput).NotTo(o.BeEmpty())
		e2e.Logf("%s", listDupAclOutput)
		...
		_ = oteutils.WaitForPodWithLabelReady(oc, "openshift-ovn-kubernetes", "app=ovnkube-node")
		ovnKNodePod, ovnkNodePodErr = oteutils.GetOVNKPodOnNode(oc, "openshift-ovn-kubernetes", "app=ovnkube-node", nodeName)
		o.Expect(ovnkNodePodErr).NotTo(o.HaveOccurred())
		o.Expect(ovnKNodePod).ShouldNot(o.Equal(""))
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote][FdpOvnOvs] 68660-Exposed route of the service should be accessible when allowing inbound traffic from any namespace network policy is created.", func() {
		var (
			buildPruningBaseDir             = testdata.FixturePath("networking")
		...
		g.By("Create a network policy in namespace")
		oteutils.CreateResourceFromFile(oc, ns, allowFromAllNSNetworkPolicyFile)
		output, err := oc.Run("get").Args("networkpolicy").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote] 75540-Network Policy Validation", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
		...
		oc.SetupProject()
		ns := oc.Namespace()
		o.Expect(oteutils.CreateResourceFromFileWithError(oc, ns, networkPolicyFile)).To(o.HaveOccurred())
	})

	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote] 70009-Pod IP is missing from OVN DB AddressSet when using allow-namespace-only network policy", func() {
		var (
			buildPruningBaseDir          = testdata.FixturePath("networking")
		...
		oteutils.CreateResourceFromFile(oc, ns, allowSameNSNetworkPolicyFile)
		output, err := oc.AsAdmin().Run("get").Args("networkpolicy", "-n", ns).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("allow-same-namespace"))

		ovnNodePod := oteutils.OvnkubeNodePod(oc, nodeList.Items[0].Name)
		...
		o.Eventually(func() string {
			result, err := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnNodePod, "ovnkube-controller", listPGCmd)
			if err != nil {
		...
		PGCMap := oteutils.NbContructToMap(listPGCOutput)
		acls := strings.Split(strings.Trim(PGCMap["acls"], "[]"), ", ")
		o.Expect(acls).To(o.HaveLen(2))

		listAclCmd := fmt.Sprintf("ovn-nbctl list acl %s", strings.Join(acls, " "))
		listAclOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnNodePod, "ovnkube-controller", listAclCmd)
		o.Expect(listErr).NotTo(o.HaveOccurred())
		o.Expect(listAclOutput).NotTo(o.BeEmpty())
		...
		for _, addrSetName := range trimmedAddrSetNames {
			...
			listAddrOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnNodePod, "ovnkube-controller", listAddressSetCmd)
			o.Expect(listErr).NotTo(o.HaveOccurred())
			o.Expect(listAddrOutput).NotTo(o.BeEmpty())
			AddrMap := oteutils.NbContructToMap(listAddrOutput)
			addrs := strings.Trim(AddrMap["addresses"], "[]")
			o.Expect(addrs).To(o.BeEmpty())
		}

		g.By("5. Create a hello pod on non existent node")
		nonexistNodeName := "doesnotexist-" + oteutils.GetRandomString()
		pod1 := oteutils.PingPodResourceNode{
			Name:      "hello-pod",
			Namespace: ns,
		...
		ipStack := oteutils.CheckIPStackType(oc)
		Pod1IP, Pod1IPv4 := oteutils.GetPodIP(oc, ns, pod1.Name)
		ovnNodePod = oteutils.OvnkubeNodePod(oc, nodeList.Items[0].Name)
		for _, addrSetName := range trimmedAddrSetNames {
			listAddressSetCmd := fmt.Sprintf("ovn-nbctl list address_set %s", addrSetName)
			listAddrOutput, listErr := oteutils.RemoteShPodWithBashSpecifyContainer(oc, "openshift-ovn-kubernetes", ovnNodePod, "ovnkube-controller", listAddressSetCmd)
			...
			if ipStack == "ipv6single" || ipStack == "ipv4single" {
				o.Expect(addrs).To(o.Equal(Pod1IP))
			} else if strings.Contains(addrSetName, "_v4") {
				o.Expect(addrs).To(o.Equal(Pod1IPv4))
			} else {
				o.Expect(addrs).To(o.Equal(Pod1IP))
			}
		}
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote] 83735-Verify NP with label npprotection/blockdeletion=true label can not be deleted after VAP is applied.", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("networking")
		...
		for i, ns := range allNS {
			oteutils.CreateResourceFromFile(oc, ns, networpolicyFiles[i])
			output, err := oc.AsAdmin().Run("get").Args("-n", ns, "networkpolicy").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(strings.Contains(output, npNames[i])).To(o.BeTrue())
			defer func() {
				_ = oc.AsAdmin().Run("label").Args("-n", ns, "networkpolicy", npNames[i], fmt.Sprintf("%s-", matchLabelKey)).Execute()
			}()
		...
		defer func() {
			_ = oc.AsAdmin().WithoutNamespace().Run("adm").Args("policy", "remove-cluster-role-from-user", "cluster-admin", oc.Username()).Execute()
		}()
		err := oc.AsAdmin().WithoutNamespace().Run("adm").Args("policy", "add-cluster-role-to-user", "cluster-admin", oc.Username()).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		defer oteutils.RemoveResource(oc, true, true, "validatingadmissionpolicybinding", vapName+"-binding")
		defer oteutils.RemoveResource(oc, true, true, "validatingadmissionpolicy", vapName)
		...
		g.By("5. Delete the VAP and its binding, verify the networkpolicy can be deleted.")
		oteutils.RemoveResource(oc, true, true, "validatingadmissionpolicybinding", "fine-grained-network-policy-protection-binding")
		oteutils.RemoveResource(oc, true, true, "validatingadmissionpolicy", "fine-grained-network-policy-protection")
...
var _ = g.Describe("[sig-network] SDN networkpolicy", func() {
	...
	g.It("[JIRA:Networking][OTP][ovn-kubernetes-ote] 69234-high memory usage on ovnkube-master leader pods on some clusters when a network policy is deleted.", g.Serial, func() {
		var (
			buildPruningBaseDir           = testdata.FixturePath("networking")
		...
		ovnMasterPodName := oteutils.GetOVNKMasterPod(oc)
		ovnMasterPodNames := oteutils.GetPodName(oc, "openshift-ovn-kubernetes", "app=ovnkube-control-plane")
		var port string
		...
		checkErr := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 100*time.Second, false, func(_ context.Context) (bool, error) {
		...
		networkPolicyResource := oteutils.NetworkPolicyResource{
		...
		for i := 0; i < 15; i++ {
			...
			oteutils.RemoveResource(oc, true, true, "networkpolicy", networkPolicyResource.Name, "-n", ns)
		}
		...
		o.Expect(string(PreGoroutineOut)).To(o.Equal(string(PostGoroutineOut)))

		g.By("6. Verify ovnk-master pod memory usage should be the same as previous.\n")
		// wait for ovnk-master leader pod to be stable
		checkErr = wait.PollUntilContextTimeout(context.Background(), 10*time.Second, 300*time.Second, false, func(_ context.Context) (bool, error) {

...
kcli create vm -i fedora42 %[1]s --wait -P cmds=['dnf install -y docker','systemctl enable --now docker']
`, testMachineName))
...
func showTestMachine() (*Machine, error) {
	...
	cmd := exec.Command("kcli", "show", "vm", testMachineName, "-o", "json")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func removeTestMachine() error {
	cmd := exec.Command("kcli", "remove", "-y", "vm", testMachineName)
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func networkInterfaceFromNode(nodeName, networkName string) (*api.NetworkInterface, error) {
	machineName := nodeNameToMachineName(nodeName)
	nodeMachine, err := showMachine(machineName)
	if err != nil {
		return nil, fmt.Errorf("failed to find vm %q: %w", machineName, api.NotFound)
	}

	machineNetwork := nodeMachine.findNetwork(networkName)
	if machineNetwork == nil {
		return nil, fmt.Errorf("missing network %q at machine %q", networkName, machineName)
	}
	if machineNetwork.Mac == "" {
		return nil, fmt.Errorf("missing mac for network %q at machine %q", networkName, machineName)
	}
	nns, err := getNodeNetworkState(nodeName)
	if err != nil {
		return nil, fmt.Errorf("failed to get node network state for node %q: %w", nodeName, err)
	}
	iface := nns.FindInterfaceByMAC(machineNetwork.Mac)
	if iface == nil {
		return nil, fmt.Errorf("failed to find interface with MAC %q in node network state for node %q", machineNetwork.Mac, nodeName)
	}
...
func nodeNameToMachineName(nodeName string) string {
